...
- The versioned v3 successor now binds all 43,008 ordered candidate metadata records, the exact
  21,504 selected / 21,504 discarded partition, and 10,752 exact / 10,752 single-fault assignment
  sets. Its 28,108,762-byte receipt was independently regenerated byte-for-byte and survived the
  constructor, mutation, omission, reorder, namespace, helper, claim, source, and runtime attacks
  that invalidated attempt 1. The narrow go receipt is
  `experiments/runs/20260805-0440-action-correction-roster-audit-s17/attempt-2-go.json`, SHA-256
  `e80e1a87769671ca83181c5d8840125ea0d3b6ebebd5e80f748307fc28733194`.
- This fixes metadata completeness only. The catalog remains one global placeholder and explicitly
  reports `per_candidate_semantic_coordinates_bound=false`; no row was rendered or materialized.
  A v4-or-later allocation basis must bind real candidate-level semantics before this roster can
  contribute to a population, model, CUDA, JarvisLabs, training, or launch decision.
...
### 2026-08-05 addendum — guarded wrapper bytecode does not bind its callable closure

- The second phase/scoring revision correctly rederived the intended false-action, catastrophic,
  unsupported-P, D-roster, and checkpoint facts on the honest path. Both public boundary wrappers,
  however, captured `check` and `function` closure cells.
- An independent audit replaced only the mutable `function` cell. The checked wrapper object and
  bytecode identities did not change, both entry and exit checks passed, and each boundary emitted
  all 14 authorization flags true plus forged launch and phase-transition claims. Restoring the
  original cells restored the all-false outputs.
- The exact no-go is
  `experiments/runs/20260805-0450-action-correction-phase-scoring-audit-s17/attempt-2-no-go.json`,
  SHA-256 `69e548f92b492f6aa42487110e9dbbfdbcff39c24d2f124a14729b5f69a6f024`.
  Accepted evidence-producing APIs must be closure-free or must bind and validate every closure
  cell; wrapper identity plus bytecode is not enough. This remains a fail-closed same-UID integrity
  mechanism, not an external security boundary.

- The closure-free v4 successor still relied on a hand-curated dependency list and separately
  mutable module-level entry/exit guards. Under an isolated defensive review, five P reports whose
  runtime joins were genuinely incomplete were classified as joined and passing, and the phase
  advanced to `P_PASS`. The same review showed that unchanged public boundaries could emit true
  authorization when both guard names changed. Green honest-namespace tests did not cover that
  trusted dependency surface.
- V4 also treated checkpoint provenance as a digest echo: `RunScore` bound neither checkpoint SHA
  nor model-run config, P reports were not linked to frozen C17, and D predictions were not linked
  to a member of the claimed checkpoint manifest. The attempt-3 no-go is
  `experiments/runs/20260805-0450-action-correction-phase-scoring-audit-s17/attempt-3-no-go.json`,
  SHA-256 `485c696f39dc721a4db9c419d6b948d7c6bf1c8b9e23290c8ce631b1c222f794`.
  A successor needs mechanically complete runtime dependency coverage plus model-execution lineage,
  not another broader manual list.
...
- The versioned v2 firewall closed those ordinary object and module attacks, but its independent
  audit changed the mutable closure cell captured by `DuplicateFirewallReceipt.to_record()`.
  Function identity and bytecode remained unchanged while the same honest receipt serialized
  `firewall_passed=true`, `scientific_firewall_passed=true`, and seven true authorization flags.
  Restoring the cell restored the honest false receipt. The attempt-2 no-go is
  `experiments/runs/20260805-0500-action-correction-duplicate-firewall-audit-s17/attempt-2-no-go.json`,
  SHA-256 `3ea364d7ede075de2a48c89d416b62d7639d65f399ec53d30bf355626abc5d11`.
- Closure contents are mutable runtime dependencies too; checking only a wrapper object and its
  bytecode is insufficient. The next revision must bind closure names, cell identities, and safe
  immutable cell contents at every entry and exit. Compact and Unicode role markers also need a
  fail-closed rule rather than another incomplete lexical blacklist.

- The closure-free v3 successor still used two directly imported iterator helpers without including
  their identities in the runtime snapshot. One controls complete pair enumeration and the other
  controls semantic-lineage/duplicate adjacency. Under changed adjacency behavior, an honestly
  rejected shared T/P family emitted a three-component, zero-edge receipt claiming complete scan
  coverage; entry and exit validation did not notice. All authorization and scientific flags stayed
  false, which limited impact but did not make the structural evidence valid.
- The attempt-3 no-go is
  `experiments/runs/20260805-0500-action-correction-duplicate-firewall-audit-s17/attempt-3-no-go.json`,
  SHA-256 `8ab2deffcb099fb1b743bc7565eb81af207f642ea316b9105282454b23283f29`.
  Integrity manifests must be checked against the global names actually referenced by evidence
  code; a manually curated subset repeatedly misses semantic dependencies.

### 2026-08-05 addendum — coordinate uniqueness is not a semantic catalog

- Semantic-catalog v1 used injective affine maps, but T covered the full tuple domain, all P/D
  tuples appeared in T, and P/D shared 24 tuples. D's temporal/context axes were materially skewed.
  The config named axes without binding their value tables, stratum render rules, compiler/schema/
  policy/timezone identities, or tests proving which variants change meaning rather than style.
- Its fault taxonomy also mixed 32 P clarification-slot controls into the advertised 448 argument
  faults; strict tool-argument faults total 416. ABSTAIN's one-leaf invalid CALL has no distinct
  reference transition, and PAUSE_MEDIA's decision mutation changes policy behavior rather than
  the reference effect. These must be separate measured classes.
- One logical source batch per role collapses every nominal row to at most one connected component.
  A literal all-pairs ledger for 43,008 rows would require 924,822,528 pairs and over 508 GB even
  under the current minimum schema, while the implemented bounded firewall handles 256 records.
- The immutable design no-go is
  `experiments/runs/20260805-0530-action-correction-semantic-catalog-audit-s17/attempt-1-no-go.json`,
  SHA-256 `bfc67650153a885145648aad2f0eac4029b01994bfbef75ee97fa1467f857c13`.
  Never implement or patch v1. A distinct v2 must bind exact axis tables and dependencies, correct
  the fault classes and denominators, provide exact P runtime support, use honest provenance, and
  replace a negative-pair ledger with completeness-proven sharded or indexed coverage.

### 2026-08-05 addendum — a balanced catalog still needs executable semantics

- Semantic-catalog v2 fixed the earlier product, role-ID, fault-denominator, and all-pairs design
  errors, but encoded multi-argument tools in a hand-written order. Ten of thirteen literal orders
  differed from the canonical simulator registry even though the design forbade reordering. It also
  omitted each role's visible-name field label and the exact list-to-object projection.
- Ten strata named too few inputs to derive required IDs, a route mode, or message channel; no
  complete world-precondition table filled the gap. The unconditional context product yielded
  1,344 nominal exact `PAUSE_MEDIA` candidates with no playing session. Three relative-time cases
  also disagreed with the apparent world-clock offset, and `PROPOSE_MESSAGE` versus `CONFIRM` had
  no bound program or decision difference.
- The independent v2 no-go is
  `experiments/runs/20260805-0535-action-correction-semantic-catalog-v2-prefreeze-s17/attempt-1-no-go.json`,
  SHA-256 `1a984583bee707dcb3de76b9d8caf8ebae806837351b9fdc990ff600c98b07b2`.
  Static balance is not a semantic certificate. A successor must bind executable request/world/
  program/gold/policy derivations to the actual simulator and test real reference transitions
  before enumerating a population.

### 2026-08-05 addendum — freeze source-dependent contracts only after the source is final

- Sub-100M product-benchmark run `20260805-1352-mobile-sub100m-ots-s17` froze lane-contract SHA
  `983cfd95552db9f59c3a0469cd4012af38e289ad2160236f00b7911582216d12`, which embedded an
  obsolete runner hash. The security fixes required by independent audit necessarily changed both
  files, making the registered binding impossible to satisfy honestly.
- The same preregistration named three authority receipts without freezing their exact schemas and
  omitted the aggregate qualification receipt's schema-version semantic. Hash-shaped placeholders
  are not an authorization boundary, and an external wrapper cannot repair a missing frozen source
  contract after registration.
- The run closed before network, Hugging Face, model, tokenizer, target, CUDA, JarvisLabs,
  generation, score, or official-961 access. Its closure receipt SHA-256 is
  `3c23a98f9010ea75a42de9707164955055cf3c41b2cfc1aa9fdfd3145ae35eec`.
- For a successor, finish and independently audit the public runner first, commit it cleanly, then
  freeze every source-dependent lane, receipt, lifecycle, result, and complete-table schema. Never
  refresh an embedded hash inside an already frozen run merely to make the gate pass.

### 2026-08-05 addendum — public demos must test the exact schema presentation users receive

- The first BarunAction-35M public-demo environment setup stopped because the clean Ubuntu VM did
  not include `python3-venv`. The model and web service had not started. Install that standard
  system dependency before creating the isolated environment on a fresh JarvisLabs CPU VM.
- The email showcase was exact when only its relevant `send_email` declaration was supplied, but
  failed closed with a malformed proposal when the demo silently presented all seven tools. Map,
  contact, Wi-Fi, and flashlight examples happened to pass both presentations. A curated browser
  example must bind and test its actual caller-visible schema rather than borrow evidence from a
  different prompt.
- The deployed UI now makes the action type explicit, binds one relevant schema per curated
  example, leaves schemas editable under advanced inputs, and preserves schema failures instead of
  repairing them. All five exact public flows passed through the external browser endpoint. Full
  evidence is in
`experiments/runs/20260805-1758-barunaction-public-demo-v1/result.json`.

### 2026-08-05 addendum — temporary demo transport is not a release hostname

- After the public-demo service was redesigned and restarted, each Gradio-managed share hostname
  returned HTTP 404 even though the exact JarvisLabs VM, local HTTP service, and FRP client stayed
  healthy. A generated tunnel URL is not evidence of public reachability; root, config, real
  inference, and feedback must be tested through the final external hostname after every transport
  change.
- The replacement anonymous localhost.run SSH/TLS tunnel passed those checks, but its hostname can
  rotate after reconnect. It is a shareable preview, not a permanent production address. A stable
  registered hostname remains the release-hosting blocker.
- The first feedback implementation retained the raw current request and Action IR automatically.
  That exceeded the minimum needed for public failure reports and could capture contact details.
...
## 2026-08-05 — mobile scale-sweep attempt-6 controller death + premature denylist pause block

- After `jl create` returned exact H200 **465257** (`barun-scale-sweep-a6-20260805`, template=axolotl), the initial local `safe_run` controller died before `machine_ssh_ready` with empty stdout/stderr (likely shell-session teardown of a non-detached background process). No second GPU was created.
- Recovery used `owned_continue_attempt_6.py` on the same Running exact ID: attested axolotl/CPython 3.11.10, bound the unbound attempt inventory, launched with `--isolated-project-venv`, completed the scientific sweep (`r_7809186b`, exit 0), and downloaded `essential-attempt-6/`.
- A mid-run write added **465257** to `infra/jarvis/protected-resources.json` before pause, so `pause_owned` failed closed with "permanently protected". Fix: briefly remove that one ID, pause-verify, then restore it as paused evidence. Lesson: do not permanently denylist an in-flight owned create until after pause-verify; prefer a non-blocking annotation or post-pause protection.
- Packaging lesson retained from attempts 1/4: template must be **axolotl**, and launch must use **`--isolated-project-venv`** (`include-system-site-packages=false`; `flash_attn` ModuleNotFoundError).
